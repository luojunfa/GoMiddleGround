...
var(
	...
	myLog    *log.Logger
	CheckTime= flag.String("CheckTime",time.Now().AddDate(0,0,-1).Format("2006-01-02"),"date")//获取前一天的时间,运行就计算
)
// 数据库连接信息
...
func ALLPeopleEnterGame(Robot bool,SmallMatch bool) []AllMatchUserNum {
	...
	sql := wxgameDB.Debug().Table("match_user_info")
		...
		if err := sql.Where("robot=? and log_time=?", Robot,CheckTime).Group("match_id,log_date").Find(&a).Error; err != nil {
